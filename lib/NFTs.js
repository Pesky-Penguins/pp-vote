...
export async function getMetadataForMint(connection, tokenAddress) {
  try {
...
export async function getNFTDataForMint(connection, mintAddress) {
...
export async function getNFTsForWallet(connection, walletAddress, verifiedCreatorAddress) {
  ...
  const mints = await Promise.all(
    response.value
      ...
      .map((accInfo) => getMetadataForMint(connection, accInfo.account.data.parsed.info.mint))
  );
